...
def _binary_label(trg: int, binary_rule: str) -> int:
    if binary_rule == "trg0_vs_123":
        ...
        if trg in (2, 3):
            return 0
    raise ValueError(f"Unsupported TRG/rule combination: trg={trg!r}, binary_rule={binary_rule!r}")


def discover_images(data_root: Path) -> dict[str, Path]:
    return {path.stem: path for path in sorted((data_root / "Dicom").glob("*.npy"))}


def discover_masks(data_root: Path) -> dict[str, Path]:
    return {_case_id_from_mask_name(path): path for path in sorted((data_root / "labels").glob("*_label.npy"))}
...
def build_manifest(
    ...
    *,
    ...
    case_col = _find_column(labels.columns, "CRF")
    trg_col = _find_column(labels.columns, "TRG")
    ...
    for _, record in labels.iterrows():
        ...
        if require_mask and not mask_shape_matches:
            continue
        rows.append(
            ManifestRow(
                case_id=case_id,
                ...
                mask_path=mask_path,
                trg=trg,
                binary_label=_binary_label(trg, binary_rule),
